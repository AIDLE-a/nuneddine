...
function AiReportCard({ stock, analysis, isLoading }) {
  ...
  const financialItems = financial ? [
    {
      label: 'PER (주가수익비율)',
      value: financial.per ? `${financial.per.toFixed(2)}배` : (financial.forward_per ? `${financial.forward_per.toFixed(2)}배 (예상)` : '–'),
      desc: '낮을수록 저평가',
      good: financial.per ? financial.per < 15 : null,
    },
    {
      label: 'PBR (주가순자산비율)',
      value: financial.pbr ? `${financial.pbr.toFixed(2)}배` : '–',
      desc: '1 미만이면 자산 대비 저평가',
      good: financial.pbr ? financial.pbr < 1 : null,
    },
    {
      label: 'ROE (자기자본이익률)',
      value: financial.roe ? `${(financial.roe * 100).toFixed(1)}%` : '–',
      desc: '높을수록 수익성 좋음',
      good: financial.roe ? financial.roe > 0.1 : null,
    },
    {
      label: '부채비율',
      value: financial.debt_to_equity ? `${financial.debt_to_equity.toFixed(1)}%` : '–',
      desc: '낮을수록 재무 안정적',
      good: financial.debt_to_equity ? financial.debt_to_equity < 100 : null,
    },
    {
      label: '매출 성장률',
      value: financial.revenue_growth ? `${(financial.revenue_growth * 100).toFixed(1)}%` : '–',
      desc: '전년 대비 매출 증가율',
      good: financial.revenue_growth ? financial.revenue_growth > 0 : null,
    },
    {
      label: '영업이익 성장률',
      value: financial.earnings_growth ? `${(financial.earnings_growth * 100).toFixed(1)}%` : '–',
      desc: '전년 대비 영업이익 증가율',
      good: financial.earnings_growth ? financial.earnings_growth > 0 : null,
    },
    {
      label: '영업이익률',
      value: financial.operating_margin ? `${(financial.operating_margin * 100).toFixed(1)}%` : '–',
      desc: '높을수록 수익성 좋음',
      good: financial.operating_margin ? financial.operating_margin > 0.1 : null,
    },
    {
      label: '유동비율',
      value: financial.current_ratio ? `${financial.current_ratio.toFixed(2)}` : '–',
      desc: '1.5 이상이면 단기 안정적',
      good: financial.current_ratio ? financial.current_ratio > 1.5 : null,
    },
  ] : [];
          ...
          <>
            ...
            {analysis && analysis.volume_analysis && (
              <div className="report-volume-box" style={{
                backgroundColor: 'var(--card-hover-bg, #f3f4f6)',
        ...
        >
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              ...
              <button
                onClick={() => setShowFinancial(false)}
                style={{ width: 28, height: 28, borderRadius: '50%', background: '#f3f4f6', border: 'none', cursor: 'pointer', fontSize: 14, color: '#6b7280', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
              >✕</button>
            </div>
              ...
              {financialItems.map((item, i) => (
                <div
                  ...
                  style={{
                    ...
                    padding: '14px 12px',
                    borderBottom: i < financialItems.length - 1 ? '1px solid #f3f4f6' : 'none',
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    {item.good !== null && item.value !== '–' && (
                      <span style={{
                        ...
                        background: item.good ? '#dcfce7' : '#fee2e2',
                        color: item.good ? '#16a34a' : '#dc2626',
                        fontWeight: 500,
                      }}>
                        {item.good ? '양호' : '주의'}
                      </span>
                    )}
                    <span style={{ fontSize: 15, fontWeight: 700, color: '#111827', minWidth: 80, textAlign: 'right' }}>
                      {item.value}
                    </span>
                  </div>
                </div>
