...
| 팀원 | 역할 | 담당 코드 |
|------|------|-----------|
| 채민 | FE 리더 — React 대시보드, 라우팅, Firebase 연동 | `FE/src/` |
| 희선 | BE 리더 — FastAPI 오케스트레이터, Prophet 예측, Critic 에이전트 | `BE/main.py`, `BE/services/prediction_service.py`, `BE/critic.py` |
...
│   │   └── nune-icon.svg            # 앱 아이콘
│   ├── src/
│   │   ├── App.jsx                  # 루트 — React Router + 전역 상태관리
...
│   │       ├── FavoritesPage.jsx    # 관심 종목 페이지 (/favorites)
│   │       ├── HistoryPage.jsx      # 분석 기록 페이지 (/history)
│   │       └── MyPage.jsx           # 마이페이지 (/mypage)
│   ├── jsconfig.json
│   └── package.json
│
└── BE/                              # FastAPI 백엔드
    ├── main.py                      # 통합 서버 (로그인 + 분석 + 검색 + 연관종목)
    ├── schemas.py                   # 팀 공통 데이터 계약 (Pydantic)
    ├── critic.py                    # Critic 에이전트 — 최종 신뢰도 산출
    ├── yubin_data.py                # 종목 키워드 맵
...
| `GET` | `/api/search?q=오뚜기` | 전 세계 종목 검색 (한국어 지원) |
